...
export function getSubbotConfig(botJid, defaultConfig) {
    ...
    try {
        ...
        if (cleanNumber && data[cleanNumber]) {
            ...
            return {
                name: subData.name || defaultConfig?.botName || config?.botName || 'tᥱwιᥲᥒιx',
                ownerName: subData.ownerName || defaultConfig?.ownerName || 'sᥲtsυkι tᥲᥴhιbᥲᥒᥲ',
        ...
        return {
            name: defaultConfig?.botName || config?.botName || 'tᥱwιᥲᥒιx',
            ownerName: defaultConfig?.ownerName || 'sᥲtsυkι tᥲᥴhιbᥲᥒᥲ',
...
export function validateSubbotOwner(m, conn) {
    ...
    if (!isSubbot) {
        return {
            allowed: false,
            reason: '*Este comando solo se puede utilizar en la sesion Jadibot.*'
        }
    }
    ...
    if (!isMainOwner && !isSubbotOwner) {
        return {
            allowed: false,
            reason: '> *Solo el creador y el Jadibot pueden acceder a este apartado.*'
        }
    }
